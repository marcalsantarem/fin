...
import {
  ...
  FolderTree, Monitor, Palette, Pencil, Plus, ReceiptText, RefreshCw, Search, Settings, ShieldCheck, Sun,
...
type ThemeId = "classic" | "atelier" | "pulse" | "lumen" | "aurora";
type ColorMode = "light" | "dark" | "system";

const themeIds: ThemeId[] = ["classic", "atelier", "pulse", "lumen", "aurora"];
...
function CategoriesScreen({ categories, onNew, onNewSubcategory, onEdit, onActive, onRemove }: { categories: Category[]; onNew: () => void; onNewSubcategory: (parent: Category) => void; onEdit: (category: Category) => void; onActive: (category: Category) => void; onRemove: (category: Category) => void }) {
  ...
  return <>
    ...
    {roots.length ? <>
      <section className="category-guide panel"><span><FolderTree size={22} /></span><div><strong>Suas subcategorias ficam dentro da categoria principal</strong><p>Abra um grupo abaixo para visualizar, editar ou criar subcategorias.</p></div><div className="category-stats"><span><strong>{roots.length}</strong> principais</span><span><strong>{childCount}</strong> subcategorias</span></div></section>
    ...
    </> : <section className="panel"><EmptyState title="Nenhuma categoria" description="O gatilho de cadastro deveria criar as categorias padrão." action="Nova categoria" onAction={onNew} /></section>}
  </>;
}
...
function EntityModal({ kind, item, parentId, accounts, cards, categories, onClose, onSave }: { kind: EntityKind; item?: EditableEntity; parentId?: string; accounts: Account[]; cards: CreditCardRow[]; categories: Category[]; onClose: () => void; onSave: (value: EntityForm) => Promise<void> }) {
  ...
  const today = new Date().toISOString().slice(0, 10); const base: EntityFormInput = { type: kind === "account" ? "digital" : requestedParent?.type ?? "expense", parentId: parentId ?? "", color: requestedParent?.color ?? "#315c4d", frequency: "monthly", date: today, closingDay: 20, dueDay: 25, day: new Date().getDate(), count: 2 };
  ...
  const { register, handleSubmit, control, setValue, formState: { errors, isSubmitting } } = useForm<EntityFormInput, unknown, EntityForm>({ resolver: zodResolver(entitySchema), defaultValues: base }); const entityType = useWatch({ control, name: "type" }); const selectedParentId = useWatch({ control, name: "parentId" });
  ...
  return <Modal eyebrow={item ? "EDITAR" : "NOVO"} title={modalTitle} subtitle={requestedParent ? `Será organizada dentro de ${requestedParent.name}.` : "Os dados serão protegidos pelas políticas RLS."} onClose={onClose}><form onSubmit={handleSubmit(onSave)}>
...
function AuthScreen({ mode, supabase, onSwitch }: { mode: "login" | "register"; supabase: SupabaseClient; onSwitch: () => void }) {
  ...
  return <div className="auth-shell"><section className="auth-visual"><Link className="brand light" href="/">FIN</Link><div className="auth-message"><span className="eyebrow">SEU DINHEIRO, SUA INTERFACE</span><h1>Finanças que<br />combinam com você.</h1><p>Organize sua vida financeira com segurança e escolha entre cinco experiências visuais completas.</p><div className="auth-theme-showcase" aria-hidden="true"><span className="showcase-classic"><i /><b>Clássico</b></span><span className="showcase-atelier"><i /><b>Ateliê</b></span><span className="showcase-pulse"><i /><b>Pulse</b></span><span className="showcase-lumen"><i /><b>Lumen</b></span><span className="showcase-aurora"><i /><b>Aurora</b></span></div><div className="auth-proof"><span className="auth-mini-card"><ShieldCheck /><i><small>Privacidade</small><strong>RLS por usuário</strong></i></span><span className="auth-mini-card"><RefreshCw /><i><small>Preferências</small><strong>Sincronizadas</strong></i></span></div></div><small className="auth-footer">© 2026 FIN · Seus dados, seu controle.</small></section><section className="auth-form-wrap"><form className="auth-form" onSubmit={submit}><div className="auth-mobile-brand">FIN</div><span className="eyebrow">{registerMode ? "COMECE AGORA" : "BEM-VINDO DE VOLTA"}</span><h2>{registerMode ? "Crie sua conta" : "Acesse sua conta"}</h2><p>{registerMode ? "Um perfil, cinco temas e seus dados protegidos." : "Continue de onde parou, em qualquer dispositivo."}</p><button className="google-button" type="button" disabled={Boolean(loading)} onClick={() => void google()}>{loading === "google" ? <LoaderCircle size={18} className="spin" /> : <b className="google-mark">G</b>}Continuar com Google</button><div className="auth-divider"><span>ou use e-mail e senha</span></div>{registerMode && <FormField label="Nome"><input required maxLength={80} autoComplete="name" value={name} onChange={(event) => setName(event.target.value)} placeholder="Seu nome completo" /></FormField>}<FormField label="E-mail"><input type="email" required maxLength={254} autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="voce@email.com" /></FormField><FormField label="Senha"><div className="password-input"><input type={visible ? "text" : "password"} required minLength={registerMode ? 8 : 6} autoComplete={registerMode ? "new-password" : "current-password"} value={password} onChange={(event) => setPassword(event.target.value)} placeholder="••••••••" /><button type="button" onClick={() => setVisible(!visible)} aria-label={visible ? "Ocultar senha" : "Mostrar senha"}>{visible ? <EyeOff size={18} /> : <Eye size={18} />}</button></div></FormField>{!registerMode && <div className="auth-options"><span /><button type="button" onClick={reset}>Esqueci a senha</button></div>}{error && <div className="auth-alert error">{error}</div>}{notice && <div className="auth-alert success">{notice}</div>}<button className="primary-button auth-submit" type="submit" disabled={Boolean(loading)}>{loading === "email" ? <LoaderCircle size={18} className="spin" /> : registerMode ? "Criar conta" : "Entrar"}<ArrowRight size={18} /></button><div className="auth-switch">{registerMode ? "Já tem uma conta?" : "Ainda não tem uma conta?"}<button type="button" onClick={onSwitch}>{registerMode ? "Entrar" : "Criar conta"}</button></div><div className="secure-note"><ShieldCheck size={15} />Cada usuário acessa somente os próprios dados.</div></form></section></div>;
}
