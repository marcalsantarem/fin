...
import {
  ...
  Palette, Pencil, Plus, ReceiptText, RefreshCw, Search, Settings, ShieldCheck, Sun,
...
type EntityComposer = { kind: EntityKind; item?: EditableEntity } | null;
...
const entitySchema = z.object({
  ...
  if (value.amount !== undefined && value.amount <= 0) context.addIssue({ code: "custom", path: ["amount"], message: "Informe um valor maior que zero" });
...
export function FinApp({ initialSection = "dashboard", openComposer = false }: { initialSection?: string; openComposer?: boolean }) {
  ...
  useEffect(() => { const timer = window.setTimeout(() => { setDark(window.localStorage.getItem("fin-theme") === "dark"); setThemeReady(true); }, 0); return () => window.clearTimeout(timer); }, []);
  useEffect(() => { if (!themeReady) return; document.documentElement.dataset.theme = dark ? "dark" : "light"; window.localStorage.setItem("fin-theme", dark ? "dark" : "light"); }, [dark, themeReady]);
  ...
  const saveEntity = async (composerState: Exclude<EntityComposer, null>, value: EntityForm) => {
    ...
    try {
      ...
      if (kind === "card") {
        const payload = { name: value.name?.trim(), account_id: value.accountId || null, limit_amount_cents: cents(value.amount), closing_day: value.closingDay, due_day: value.dueDay, color: value.color || "#49305d" };
      ...
      if (kind === "category") {
        ...
        if (category && category.type !== value.type && (data.transactions.some((row) => row.category_id === category.id) || data.recurrences.some((row) => row.category_id === category.id))) throw new Error("O tipo não pode ser alterado enquanto a categoria estiver em uso.");
        const payload = { name: value.name?.trim(), type: value.type, color: value.color || "#718079", icon: category?.icon || "tag" };
  ...
  const changePassword = async (currentPassword: string, password: string) => {
    ...
    const { error } = await supabase.auth.updateUser({ email: user.email, current_password: currentPassword, password });
  ...
  const alerts = data.transactions.filter((transaction) => transaction.type === "expense" && transaction.due_date && ["pending", "overdue", "scheduled"].includes(transaction.status)).sort((a, b) => (a.due_date ?? "").localeCompare(b.due_date ?? "")).slice(0, 5);
    ...
    <div className="app-shell">
      ...
      <main className="main-area">
        ...
        <div className="content">
          ...
          {active === "installments" && <InstallmentsScreen groups={data.installments} transactions={data.transactions} onNew={() => setEntityComposer({ kind: "installment" })} onStatement={(item) => { setTransactionScope({ installmentId: item.id }); go("transactions"); }} />}
          ...
          {active === "categories" && <CategoriesScreen categories={data.categories} onNew={() => setEntityComposer({ kind: "category" })} onEdit={(item) => setEntityComposer({ kind: "category", item })} onActive={(item) => void setEntityActive("category", item)} onRemove={(item) => void removeEntity("category", item)} />}
          {active === "reports" && <ReportsScreen transactions={data.transactions} />}
          {active === "settings" && <SettingsScreen name={displayName} email={user.email ?? ""} emailVerified={Boolean(user.email_confirmed_at)} dark={dark} setDark={setDark} onSave={saveProfile} onPassword={changePassword} onLogout={() => void supabase.auth.signOut()} />}
        </div>
      </main>
      ...
      {entityComposer && <EntityModal kind={entityComposer.kind} item={entityComposer.item} accounts={data.accounts} cards={data.cards} categories={data.categories} onClose={() => setEntityComposer(null)} onSave={(value) => saveEntity(entityComposer, value)} />}
...
function Dashboard({ period, setPeriod, name, data, income, expense, paidExpense, pendingExpense, onNew, onNavigate }: { period: string; setPeriod: (value: string) => void; name: string; data: FinanceData; income: number; expense: number; paidExpense: number; pendingExpense: number; onNew: () => void; onNavigate: (section: Section) => void }) {
  ...
  return <>
    <section className="page-heading dashboard-heading"><div><span className="eyebrow">{new Intl.DateTimeFormat("pt-BR", { weekday: "long", day: "2-digit", month: "long" }).format(new Date()).toUpperCase()}</span><h1>Olá, {name.split(" ")[0]}.</h1><p>{income >= expense ? "Seu mês está equilibrado. Continue assim." : "Suas despesas estão acima das entradas deste mês."}</p></div><div className="heading-actions"><div className="period-control"><button onClick={() => shiftPeriod(-1)} disabled={period === periods.at(-1)} aria-label="Mês anterior"><ChevronLeft size={17} /></button><select value={period} onChange={(event) => setPeriod(event.target.value)}>{periods.map((value) => <option key={value} value={value}>{new Intl.DateTimeFormat("pt-BR", { month: "long", year: "numeric", timeZone: "UTC" }).format(new Date(`${value}-01T00:00:00Z`))}</option>)}</select><button onClick={() => shiftPeriod(1)} disabled={period === periods[0]} aria-label="Próximo mês"><ChevronRight size={17} /></button></div><button className="secondary-button" onClick={() => onNavigate("reports")}><Download size={17} />Relatórios</button></div></section>
...
function InstallmentsScreen({ groups, transactions, onNew, onStatement }: { groups: InstallmentGroup[]; transactions: TransactionRow[]; onNew: () => void; onStatement: (item: InstallmentGroup) => void }) {
...
function SettingsScreen({ name, email, emailVerified, dark, setDark, onSave, onPassword, onLogout }: { name: string; email: string; emailVerified: boolean; dark: boolean; setDark: (value: boolean) => void; onSave: (name: string) => void | Promise<void>; onPassword: (current: string, password: string) => Promise<boolean>; onLogout: () => void }) {
  ...
  return <><PageTitle eyebrow="PREFERÊNCIAS" title="Configurações" subtitle="Seu perfil sincronizado com o Supabase." /><div className="settings-layout"><div className="settings-menu" role="tablist" aria-label="Configurações"><button role="tab" aria-selected={tab === "general"} className={tab === "general" ? "active" : ""} onClick={() => setTab("general")}><Settings size={17} />Geral</button><button role="tab" aria-selected={tab === "security"} className={tab === "security" ? "active" : ""} onClick={() => setTab("security")}><ShieldCheck size={17} />Segurança</button><button role="tab" aria-selected={tab === "appearance"} className={tab === "appearance" ? "active" : ""} onClick={() => setTab("appearance")}><Palette size={17} />Aparência</button></div>
    {tab === "general" && <section className="panel settings-panel" role="tabpanel"><h2>Preferências gerais</h2><p>Atualize os dados do seu perfil.</p><div className="settings-field"><span><strong>Nome</strong><small>Exibido somente dentro da sua conta</small></span><input value={profileName} maxLength={80} autoComplete="name" onChange={(event) => setProfileName(event.target.value)} /></div><div className="settings-field"><span><strong>E-mail</strong><small>Gerenciado pelo Supabase Auth</small></span><input value={email} disabled /></div><div className="settings-actions"><button className="danger-button" onClick={onLogout}><LogOut size={17} />Sair</button><button className="primary-button" onClick={() => void onSave(profileName)}><Check size={17} />Salvar alterações</button></div></section>}
    {tab === "security" && <section className="panel settings-panel" role="tabpanel"><h2>Segurança da conta</h2><p>Altere sua senha sem expor credenciais ou tokens.</p><div className="security-status"><ShieldCheck size={19} /><span><strong>E-mail {emailVerified ? "confirmado" : "ainda não confirmado"}</strong><small>{emailVerified ? "Sua identidade de e-mail foi verificada." : "Abra a mensagem de confirmação enviada pelo Supabase."}</small></span></div><form onSubmit={submitPassword}><div className="settings-field"><span><strong>Senha atual</strong><small>Usada apenas para confirmar esta alteração</small></span><input type="password" required autoComplete="current-password" value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} /></div><div className="settings-field"><span><strong>Nova senha</strong><small>Mínimo de 8 caracteres</small></span><input type="password" required minLength={8} autoComplete="new-password" value={password} onChange={(event) => setPassword(event.target.value)} /></div><div className="settings-field"><span><strong>Confirmar nova senha</strong><small>Repita a senha escolhida</small></span><input type="password" required minLength={8} autoComplete="new-password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} /></div>{passwordError && <div className="auth-alert error">{passwordError}</div>}<div className="settings-actions"><button type="button" className="danger-button" onClick={onLogout}><LogOut size={17} />Encerrar sessão</button><button className="primary-button" type="submit" disabled={savingPassword}>{savingPassword ? <LoaderCircle className="spin" size={17} /> : <KeyRound size={17} />}Atualizar senha</button></div></form></section>}
    {tab === "appearance" && <section className="panel settings-panel" role="tabpanel"><h2>Aparência</h2><p>Esta preferência fica somente neste dispositivo.</p><div className="theme-options"><button className={!dark ? "active" : ""} onClick={() => setDark(false)}><Sun size={22} /><strong>Claro</strong><small>Superfícies claras e alto contraste.</small></button><button className={dark ? "active" : ""} onClick={() => setDark(true)}><Moon size={22} /><strong>Escuro</strong><small>Conforto visual em ambientes com pouca luz.</small></button></div><div className="security-note"><ShieldCheck size={16} />Preferências visuais não são enviadas ao banco nem contêm dados financeiros.</div></section>}
  </div></>;
}
...
function EntityModal({ kind, item, accounts, cards, categories, onClose, onSave }: { kind: EntityKind; item?: EditableEntity; accounts: Account[]; cards: CreditCardRow[]; categories: Category[]; onClose: () => void; onSave: (value: EntityForm) => Promise<void> }) {
  ...
  if (item && kind === "card") { const card = item as CreditCardRow; Object.assign(base, { name: card.name, accountId: card.account_id ?? "", amount: card.limit_amount_cents / 100, closingDay: card.closing_day, dueDay: card.due_day, color: card.color }); }
  if (item && kind === "category") { const category = item as Category; Object.assign(base, { name: category.name, type: category.type, color: category.color }); }
  ...
  const { register, handleSubmit, control, formState: { errors, isSubmitting } } = useForm<EntityFormInput, unknown, EntityForm>({ resolver: zodResolver(entitySchema), defaultValues: base }); const recurrenceType = useWatch({ control, name: "type" });
  ...
  return <Modal eyebrow={item ? "EDITAR" : "NOVO"} title={titles[kind]} subtitle="Os dados serão protegidos pelas políticas RLS." onClose={onClose}><form onSubmit={handleSubmit(onSave)}>
...
function CategorySelect({ categories, register }: { categories: Category[]; register: object }) { return <select required {...register}><option value="">Selecione</option>{categories.filter((item) => item.active).map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select>; }
